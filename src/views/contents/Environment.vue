<template>
  <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
    ...
    <template v-if="responseStatus === 404">
      <h2 class="text-center text-muted">This environment does not exist</h2>
    </template>
  ...
  methods: {
    fetchData() {
      try {
        ...
        // formData.append('rangeEndLimit', 2);
        // formData.append('rangeLimit', 2);
        // formData.append('rangeEndLimit', this.$config.RANGE_LIMIT);
        this.$axios
          .get(this.$config.BASE_URL + this.url.api.environment, {
            params: formData,
          })
          .then((response) => {
